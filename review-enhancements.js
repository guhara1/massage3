...
function setupReliableRegionSelector() {
  const root = document.querySelector("#regions");
  if (!root || root.dataset.reliableRegionSelector === "ready") return;
  ...
  if (!tabs || !citiesBox || !detailBox) return;
  root.dataset.reliableRegionSelector = "ready";

  const citySlug = { "서울":"seoul", "경기":"gyeonggi", "인천":"incheon", "부산":"busan", "대구":"daegu", "대전":"daejeon", "광주":"gwangju", "강원":"gangwon", "제주":"jeju", "세종":"sejong", "전북":"jeonbuk" };
  const districtSlug = {
    ...
    "연수 송도":"songdo", "부평":"bupyeong", "남동":"namdong", "서구":"seogu", "미추홀":"michuhol", "계양":"gyeyang", "해운대":"haeundae", "부산진":"busanjin", "수영":"suyeong", "수성":"suseong", "달서":"dalseo", "유성":"yuseong", "광산":"gwangsan", "춘천":"chuncheon", "원주":"wonju", "강릉":"gangneung", "제주시":"jeju-si", "서귀포":"seogwipo"
  };
  ...
  const gyeonggi = {
    ...
    "안양": { "만안구":["안양동","석수동","박달동"], "동안구":["관양동","비산동","평촌동","호계동"] },
    "부천": { "원미구":["중동","상동","심곡동","원미동","춘의동","도당동"], "소사구":["소사본동","송내동","괴안동","역곡동","옥길동"], "오정구":["오정동","원종동","고강동","여월동","작동"] },
    "화성 동탄":["남양읍","능동","동탄동","반송동","반월동","병점동","봉담읍","산척동","새솔동","오산동","청계동","향남읍"],
    "남양주":["다산동","별내동","오남읍","와부읍","진건읍","진접읍","퇴계원읍","평내동","호평동","화도읍"], "김포":["감정동","걸포동","고촌읍","구래동","마산동","사우동","양촌읍","운양동","장기동","풍무동"],
    "파주":["교하동","금촌동","동패동","목동동","문산읍","아동동","야당동","와동동","운정동","조리읍"], "광명":["광명동","소하동","일직동","철산동","하안동"],
    "하남":["감일동","덕풍동","망월동","미사동","신장동","위례동","창우동","풍산동"], "의정부":["가능동","금오동","녹양동","민락동","신곡동","용현동","의정부동","장암동","호원동"],
    ...
    "포천":["가산면","군내면","내촌면","소흘읍","신북면","영북면","영중면","이동면","일동면","포천동"], "가평":["가평읍","북면","상면","설악면","조종면","청평면"],
    "과천":["갈현동","과천동","막계동","문원동","별양동","부림동","원문동","중앙동"], "구리":["갈매동","교문동","동구동","수택동","아천동","인창동","토평동"],
    "군포":["광정동","군포동","궁내동","금정동","당동","대야미동","산본동","송부동","오금동","재궁동"], "동두천":["걸산동","광암동","동두천동","보산동","불현동","상봉암동","생연동","송내동","안흥동","지행동"],
    "양평":["강상면","강하면","개군면","단월면","서종면","양동면","양서면","양평읍","옥천면","용문면","지평면","청운면"], "연천":["군남면","미산면","백학면","신서면","연천읍","왕징면","장남면","전곡읍","중면","청산면"],
    "의왕":["고천동","내손동","부곡동","삼동","오전동","왕곡동","청계동","포일동"], "광주":["경안동","곤지암읍","남종면","능평동","도척면","목현동","송정동","쌍령동","오포동","초월읍","탄벌동","퇴촌면"]
  };
  ...
  const regions = {
    ...
    yeongnam: { label:"영남권", desc:"부산, 대구 주요 생활권을 선택한 뒤 세부 지역을 확인합니다.", cities: { "부산": { "해운대":["우동","중동","좌동","재송동"], "부산진":["부전동","전포동","범천동","양정동"], "수영":["광안동","남천동","민락동","망미동"] }, "대구": { "수성":["범어동","만촌동","황금동","지산동"], "중구":["동성로","삼덕동","대봉동"], "달서":["상인동","월성동","두류동","성서"] } } },
    chungcheong: { label:"충청권", desc:"대전, 세종 주요 생활권을 도시별로 나눠 확인합니다.", cities: { "대전": { "서구":["둔산동","탄방동","갈마동","관저동"], "유성":["봉명동","노은동","도룡동","전민동"], "중구":["은행동","대흥동","문화동"] }, "세종": { "신도심":["나성동","도담동","아름동","종촌동"], "조치원":["조치원읍","연서면","전의면"] } } },
    honam: { label:"호남권", desc:"광주, 전북 주요 지역을 도시별 생활권으로 나눠 확인합니다.", cities: { "광주": { "서구":["치평동","상무지구","화정동","풍암동"], "광산":["수완동","첨단","하남동","월계동"], "동구":["충장로","계림동","학동"] }, "전북": { "전주":["효자동","서신동","중화산동","혁신도시"], "익산":["영등동","모현동","부송동"], "군산":["수송동","나운동","조촌동"] } } },
  ...
  function cityData() { return regions[selectedRegion].cities[selectedCity]; }
  function districtData() { return cityData()[selectedDistrict]; }
  function scrollNear(element) {
    if (!element) return;
    const top = Math.max(0, element.getBoundingClientRect().top + window.scrollY - 96);
    window.scrollTo({ top, behavior: "smooth" });
  }
  function renderTabs() {
    tabs.innerHTML = order.map((key) => makeButton(regions[key].label, `data-region="${key}"`, key === selectedRegion)).join("");
  }
  ...
  root.addEventListener("click", (event) => {
    ...
    if (regionButton && tabs.contains(regionButton)) {
      ...
      selectedCity = Object.keys(regions[selectedRegion].cities)[0];
      renderTabs();
      renderCities();
      scrollNear(citiesBox);
      return;
    }
    ...
    if (cityButton && citiesBox.contains(cityButton)) {
      event.preventDefault();
      event.stopImmediatePropagation();
      selectedCity = cityButton.dataset.city;
      renderCities();
    ...
    if (districtButton && detailBox.contains(districtButton)) {
      ...
      const branch = districtData();
      if (Array.isArray(branch)) renderDongs();
      else renderGuList();
      scrollNear(detailBox);
      return;
    }
    ...
    if (guButton && detailBox.contains(guButton)) {
      event.preventDefault();
      event.stopImmediatePropagation();
      selectedGu = guButton.dataset.gu;
      renderDongs();
      scrollNear(detailBox);
    }
  }, true);
...
setupReviewSlider();
maskLocalReviewName();
setupReliableRegionSelector();
loadStationLinks();
setTimeout(setupReliableRegionSelector, 400);
