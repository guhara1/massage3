(() => {
  ...
  const data = {
    capital: {
      ...
      cities: {
        ...
        "경기": { note: "경기는 시·군별 이동 거리와 행정구 구조가 달라, 먼저 도시를 고른 뒤 행정구 또는 세부 생활권을 확인하는 흐름이 정확합니다.", districts: {
        ...
        "인천": { note: "인천은 송도, 청라, 부평, 미추홀처럼 생활권 성격이 달라 예약 전 위치 확인이 필요합니다.", districts: {"연수 송도":["동춘동","선학동","송도동","연수동","옥련동","청학동"],"부평":["갈산동","구산동","부개동","부평동","산곡동","삼산동","십정동","일신동","청천동"],"남동":["간석동","고잔동","구월동","논현동","도림동","만수동","서창동","장수동"],"서구":["가정동","가좌동","검암동","경서동","당하동","마전동","석남동","심곡동","연희동","청라동"],"미추홀":["관교동","도화동","문학동","숭의동","용현동","주안동","학익동"],"계양":["계산동","귤현동","동양동","박촌동","병방동","서운동","용종동","작전동","효성동"]}}
      }
    },
    yeongnam: { label: "영남권", description: "부산, 대구 주요 생활권을 선택한 뒤 세부 지역을 확인합니다.", cities: {"부산": { note: "부산은 해운대와 서면처럼 생활권별 이동 시간이 달라 예약 가능 시간 확인이 중요합니다.", districts: {"해운대":["우동","중동","좌동","재송동"],"부산진":["부전동","전포동","범천동","양정동"],"수영":["광안동","남천동","민락동","망미동"]}},"대구": { note: "대구는 수성구, 동성로, 달서권 이용 목적이 달라 코스와 시간을 나눠 확인합니다.", districts: {"수성":["범어동","만촌동","황금동","지산동"],"중구":["동성로","삼덕동","대봉동"],"달서":["상인동","월성동","두류동","성서"]}}}},
    chungcheong: { label: "충청권", description: "대전, 세종 주요 생활권을 도시별로 나눠 확인합니다.", cities: {"대전": { note: "대전은 둔산, 유성, 관저처럼 상담 수요가 다른 생활권을 먼저 확인하면 편합니다.", districts: {"서구":["둔산동","탄방동","갈마동","관저동"],"유성":["봉명동","노은동","도룡동","전민동"],"중구":["은행동","대흥동","문화동"]}},"세종": { note: "세종은 신도시 생활권과 조치원권 이동 기준이 달라 위치를 먼저 알려주는 것이 좋습니다.", districts: {"신도심":["나성동","도담동","아름동","종촌동"],"조치원":["조치원읍","연서면","전의면"]}}}},
    honam: { label: "호남권", description: "광주, 전북 주요 지역을 도시별 생활권으로 나눠 확인합니다.", cities: {"광주": { note: "광주는 상무, 첨단, 수완처럼 상권과 주거권이 달라 시간 확인이 필요합니다.", districts: {"서구":["치평동","상무지구","화정동","풍암동"],"광산":["수완동","첨단","하남동","월계동"],"동구":["충장로","계림동","학동"]}},"전북": { note: "전북은 전주와 익산, 군산의 이동 동선이 달라 예약 전 지역을 구체적으로 확인합니다.", districts: {"전주":["효자동","서신동","중화산동","혁신도시"],"익산":["영등동","모현동","부송동"],"군산":["수송동","나운동","조촌동"]}}}},
    gangwonjeju: { label: "강원·제주", description: "강원과 제주는 이동 거리와 예약 시간 변동이 커서 세부 위치 확인이 특히 중요합니다.", cities: {"강원": { note: "강원은 영서와 영동 이동 시간이 크게 달라 도시와 생활권을 함께 확인합니다.", districts: {"춘천":["퇴계동","석사동","효자동","온의동"],"원주":["무실동","단계동","혁신도시","반곡동"],"강릉":["교동","입암동","포남동","유천동"]}},"제주": { note: "제주는 제주시와 서귀포 동선 차이가 커서 예약 전 위치 안내가 꼭 필요합니다.", districts: {"제주시":["연동","노형동","이도동","아라동"],"서귀포":["중문","동홍동","서귀동","성산"]}}}}
  };
  ...
  const makeAreaUrl = (city, district, dong) => `/area/${citySlug[city] || encodeURIComponent(city)}/${districtSlug[district] || encodeURIComponent(district)}/${normalizeDong(dong)}/`;
  ...
  function getCityData() {
    return data[selectedRegion].cities[selectedCity];
  }

  function getBranch() {
    return getCityData().districts[selectedDistrict];
  }
  ...
  function renderCities() {
    ...
    selectedDistrict = Object.keys(region.cities[selectedCity].districts)[0];
    selectedGu = "";
  ...
  function renderGuList() {
    ...
    detailSummary.textContent = `${selectedDistrict}은 행정구별 생활권과 이동 동선이 달라 먼저 구를 선택한 뒤 세부 동을 확인하는 흐름이 정확합니다.`;
    detailList.innerHTML = Object.keys(branch).map((gu) => button(gu, `data-gu="${gu}"`, gu === selectedGu)).join("");
  }

  function renderDongs() {
    const branch = getBranch();
    const dongs = Array.isArray(branch) ? branch : branch[selectedGu] || [];
    const title = selectedGu ? `${selectedDistrict} ${selectedGu}` : selectedDistrict;
    detailTitle.textContent = title;
    detailSummary.textContent = `${selectedCity} ${title} 세부 지역을 선택하면 해당 동 안내 페이지로 이동합니다. 예약 전에는 정확한 위치, 희망 시간, 코스 기준을 함께 확인하는 것이 좋습니다.`;
    detailList.innerHTML = dongs.map((dong) => `<a href="${makeAreaUrl(selectedCity, selectedDistrict, dong)}" aria-label="${selectedCity} ${title} ${dong} 지역 안내">${dong}</a>`).join("");
  }
  ...
  detailList.addEventListener("click", (event) => {
    ...
    if (districtButton) {
      ...
      const branch = getBranch();
      if (Array.isArray(branch)) renderDongs(); else renderGuList();
      focusStep(detailList);
      return;
    }
    if (guButton) {
