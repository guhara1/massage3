(() => {
  ...
  function applyStaticLinks() {
    ...
    document.querySelectorAll('a[href*="area.html?"]').forEach((link) => {
      ...
      if (!info) return;
      link.setAttribute("href", areaUrl(info.city, info.district, info.dong));
  ...
  root.addEventListener("click", (event) => {
    ...
    if (districtButton) {
      const city = selectedText("#subregion-list button.is-active");
    ...
    if (regionButton) {
      const label = cleanLabel(regionButton.textContent);
      window.setTimeout(() => setMeta(`${label} 출장마사지 가능 지역 | 전국 권역별 예약 안내`, `${label} 출장마사지 가능 지역을 먼저 확인하고 주요 도시, 생활권, 예약 가능 시간과 코스 기준을 비교할 수 있습니다.`), 80);
    }
  }, true);

  if (!location.pathname.startsWith("/area/")) {
    window.setTimeout(() => setMeta(baseTitle, baseDescription), 300);
  }
})();
