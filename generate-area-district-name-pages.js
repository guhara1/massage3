...
const districtSlug = {
  ...
  '연수 송도':'songdo','송도':'songdo','부평':'bupyeong','남동':'namdong','서구':'seogu','미추홀':'michuhol','계양':'gyeyang','동구':'donggu','강화':'ganghwa','옹진':'ongjin','해운대':'haeundae','부산진':'busanjin','수영':'suyeong','동래':'dongnae','남구':'namgu','사하':'saha','수성':'suseong','달서':'dalseo','유성':'yuseong','광산':'gwangsan','춘천':'chuncheon','원주':'wonju','강릉':'gangneung','제주시':'jejusi','서귀포':'seogwipo'
};
...
function slugVariants(label, mappedSlug) {
  ...
  const add = (value) => { const slug = cleanSlug(value); if (slug) variants.add(slug); };
  variants.add(encodeURIComponent(label));
  if (mappedSlug) add(mappedSlug);
  add(romanize(label, ''));
  add(romanize(label, '-'));
  if (label.endsWith('동')) {
    const base = label.slice(0, -1);
    add(`${romanize(base, '')}dong`);
    add(`${romanize(base, '')}-dong`);
  }
  return [...variants];
}
...
console.log(`Generated ${written} district-name dong pages.`);
